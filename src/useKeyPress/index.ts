 ...
 * @param event 键盘事件
 * @param keyFilter 当前键
 * @returns {string | number | boolean} 是否激活
 */
function genFilterKey(
  ...
  for (const key of genArr) {
    ...
    // keyCode 别名
    const aliasKeyCode: number | number[]
      = aliasKeyCodeMap[key.toLowerCase() as keyof typeof aliasKeyCodeMap]
...
/**
 * A hook that handles key press events based on the provided key filter and event handler.
 *
